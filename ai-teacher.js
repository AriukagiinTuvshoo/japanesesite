...
(function(){
 ...
 function itemSummary(type,id){
   ...
   if(type==='kanji')return {contentType:type,contentId:x.id,character:x.char,level:x.level,source:x.source||'derived-from-vocabulary',relatedWords:(x.related||[]).slice(0,4).map(function(w){return {japanese:w.jp,reading:w.reading||'',meaningMn:w.mn||'',meaningEn:w.en||''};})};
 ...
 function errorMessage(error){if(!error)return t('network');if(error.code==='AI_NOT_CONFIGURED')return t('notConfigured');if(error.code==='AI_TIMEOUT')return t('timeout');if(error.code==='AI_HTTP_ERROR'||error.code==='AI_PROVIDER_ERROR'||error.code==='AI_NETWORK_ERROR')return t('serverError');if(/^INVALID_/.test(error.code||'')||error.code==='REQUEST_TOO_LARGE'||error.code==='MESSAGE_TOO_LARGE'||error.code==='CONTEXT_TOO_LARGE'||error.code==='INVALID_ACTION'||error.code==='INVALID_MODE'||error.code==='INVALID_QUESTION_COUNT'||error.code==='INVALID_CANDIDATES')return t('invalidRequest');if(error.code==='OFFLINE')return t('network');return t('serverError');}
