...
function render(){
 ...
 var pb=$('#v8-pause-btn');if(pb)pb.addEventListener('click',function(){if(window.pausePhase1Session&&window.resumePhase1Session){var label=pb.textContent;if(label.indexOf('Үргэлжл')>=0){window.resumePhase1Session();pb.textContent='Түр зогсоох'}else{window.pausePhase1Session();pb.textContent='Үргэлжлүүлэх'}}});var rb=$('#v8-review-btn');if(rb)rb.addEventListener('click',function(){location.hash=due.length?'#study-hub':'#learn'});
}
function shell(){
