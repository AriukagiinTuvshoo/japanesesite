...
(function(){
  ...
  function moduleSummary(type,state){
    var counts={NEW:0,LEARNING:0,FAMILIAR:0,WEAK:0,MASTERED:0},items=LearningData.get(type);
    items.forEach(function(item){var p=existingProgress(state,type,item.id);var status=LearningStore.getStatus(p||{});counts[status]++;});
