...
(function(){
  ...
  function questionLabel(mode){
    var map={"ja-mn":"jaToMn","mn-ja":"mnToJa",reading:"reading",sentence:"sentence","kanji-related-word":"kanjiToWord","pattern-title":"grammarMeaning","example-pattern":"exampleGrammar"};
  ...
  function mount(){
    ...
    if(!$("#study-hub")){
      home.insertAdjacentHTML("beforeend",
        ...
        "<section class='wrap p2-section' id='mistakes'><div class='p2-heading'><div><div class='eyebrow'>"+esc(t("review"))+"</div><h2>"+esc(t("mistakes"))+"</h2><p>"+esc(t("noMistakes"))+"</p></div><button class='btn primary' id='p2-review-mistakes'>"+esc(t("mistakeReview"))+" →</button></div><div id='mistake-list' class='mistake-list'></div></section>"+
        "<section class='wrap p2-section' id='progress'><div class='p2-heading'><div><div class='eyebrow'>"+esc(t("progress"))+"</div><h2>"+esc(t("progress"))+"</h2><p>"+esc(t("noProgress"))+"</p></div></div><div id='progress-overall' class='progress-overall'></div><div id='progress-modules' class='progress-module-grid'></div><div id='progress-jlpt' class='jlpt-progress'></div></section>"
      );
    }
    var routes=[
      ...
      ["practice","✓",t("practice")],["mistakes","⚠",t("mistakes")],["progress","↗",t("progress")],["ai-teacher","AI","AI先生"]
    ];
    var nav=$(".sidebar-nav");
    ...
    if(desktop)desktop.innerHTML=routes.slice(0,8).map(function(x){return "<a href='#"+x[0]+"' data-nav='"+x[0]+"'>"+esc(x[2])+"</a>"}).join("");
    var mobile=$(".mobile-nav");
    if(mobile)mobile.innerHTML=[["dashboard","⌂",language==="ja"?"ホーム":language==="en"?"Home":"Нүүр"],["study-hub","▶",t("study")],["vocabulary","語",t("vocabulary")],["practice","✓",t("practice")],["progress","↗",t("progress")],["ai-teacher","AI","AI先生"]].map(function(x){return "<a href='#"+x[0]+"' data-nav='"+x[0]+"'><span>"+x[1]+"</span><i>"+esc(x[2])+"</i></a>"}).join("");
  ...
  function drawQuiz(){
    ...
    sec.innerHTML="<div class='p2-heading'><div><div class='eyebrow'>"+esc(t("practice"))+" · "+esc(questionLabel(q.mode))+"</div><h2>"+esc(q.prompt)+"</h2><p>"+esc(questionLabel(q.mode))+"</p></div><div class='quiz-score'><b>"+quiz.correct+"</b> / "+quiz.index+"</div></div><div class='quiz-layout'><article class='quiz-panel'><div class='quiz-progress-large'><i style='width:"+Math.round(quiz.index/quiz.questions.length*100)+"%'></i></div><div class='quiz-counter'>"+(quiz.index+1)+" / "+quiz.questions.length+"</div><div class='quiz-choices'>"+q.choices.map(function(c,i){return "<button class='quiz-choice' data-c='"+i+"'><span>"+String.fromCharCode(65+i)+"</span>"+esc(c)+"</button>"}).join("")+"</div><div class='quiz-feedback' id='p2-q-feedback'></div><button class='btn primary quiz-next' id='p2-q-next' disabled>"+esc(quiz.index===quiz.questions.length-1?t("finish"):t("next"))+" →</button></article><aside class='quiz-side'><b>"+esc(t("progress"))+"</b><div>"+quiz.index+" "+esc(t("questions"))+"</div><div>"+LearningStore.getAccuracy(q.type).accuracy+"% "+esc(t("accuracy"))+"</div></aside></div>";
  ...
  function finishQuiz(){
    ...
    $("#p2-result-ai").onclick=function(){location.hash="#ai-teacher";setTimeout(function(){var wrongItems=q.results.filter(function(x){return !x.correct}).slice(0,5).map(function(x){var item=LearningData.find(x.type,x.id);return item?((item.jp||item.char||item.pattern)+""):''}).filter(Boolean);var input=document.querySelector("#ai-input");if(input)input.value=language==="mn"?"Эдгээр алдааг тайлбарла: "+wrongItems.join(", "):language==="ja"?"これらの間違いを説明してください: "+wrongItems.join(", "):"Explain these mistakes: "+wrongItems.join(", ");input&&input.focus();},50)};
  ...
  function dashboardBridge(){
    ...
    card.innerHTML="<div class='dash-card-head'><span>"+esc(t("today"))+"</span><a href='#study-hub'>"+esc(t("study"))+" →</a></div><div class='p2-dash-row'><span>♻ "+due.length+" "+esc(t("due"))+"</span><span>⏱ "+Math.floor(mins)+" / "+s.settings.dailyGoal+" "+esc(t("minutes"))+"</span></div><div class='p2-dash-mods'>"+["vocabulary","kanji","grammar"].map(function(type){var d=LearningStore.stats(type),n=d.counts.FAMILIAR+d.counts.MASTERED;return "<span><b>"+n+"</b> / "+d.total+" "+esc(t(type))+"</span>"}).join("")+"</div>";
  }
